...
export const eventQueryFnNoId = async (
...
export const eventQueryFn = async (
	/* mut */ resolved: Vantage.ResolvedEvent,
): Promise<Vantage.ResolvedEvent> => {
	...
	if (mustFetch || mustParse) await EventResolver.upsertToDatabase(resolved);
	return resolved;
};
...
export const useEventQueries = (ids: Vantage.EventId[]) =>
	useQueries({
